{*******************************************************************************
 ...
 !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
 NOTE: DO NOT use this in GExperts anymore.  We are phasing out this component.
 !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
*******************************************************************************}
unit SortGrid;
...
procedure Register;
begin
     RegisterComponents('GExperts', [TSortGrid]);
end;
...
procedure TSortGrid.UpdateScrollPage;
    function LMax(const A, B: LongWord): LongWord;
    begin
         Result:=B;
...
begin
  ...
  begin
    ...
    if GetScrollInfo(Handle, SB_VERT, SI) = True then
    begin
      SI.fMask:=SIF_PAGE;
      if ProportionalScrollBars then
        SI.nPage:=LMax(Round(((VisibleRowCount+FixedRows)/RowCount)*(SI.nMax-SI.nMin+1)), 1)
      else
        SI.nPage:=0;
  ...
  begin
    SI.cbSize:=SizeOf(SI);
    SI.fMask:=SIF_PAGE or SIF_POS or SIF_RANGE;
    if GetScrollInfo(Handle, SB_HORZ, SI) = True then
    begin
      SI.fMask:=SIF_PAGE;
      if ProportionalScrollBars then
