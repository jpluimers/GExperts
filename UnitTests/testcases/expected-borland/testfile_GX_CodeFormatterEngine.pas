...
begin
  try
    ...
    FText.Clear;
      // we can free some memory since the text is now stored in the tokens
...
begin
  ...
  if (TokenNo >= 0) and (TokenNo < FTokens.Count) then
  begin
  ...
  i := Length(Result);
  while (i > 0) and (Result[i] in [' ', Tab]) do
  begin
    Dec(i);
    SetLength(Result, i);
  end;
...
begin
  ...
  begin
    TokenNo := 0;
    while TokenNo < FTokens.Count do
    begin
