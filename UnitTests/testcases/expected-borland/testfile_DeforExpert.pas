...
begin
  ...
  try
    ...
    begin
      FFormatter := nil;
      MessageDlg(Format('%s: %s', [e.ClassName, e.Message]), mtError, [mbOK],
        0);
      Exit;
    end;
  end;
...
begin
  ...
  begin
    ...
    if Assigned(fKeyboardNotifier) then
      fKeyboardNotifier.Unregister;
    fKeyboardNotifier := TKeyboardNotifier.Create(Self,
      FWizardSettings.ShortCut);
  end;
end;
...
{$ENDIF}
    if not Assigned(Editor) or not Supports(Editor, IOTASourceEditor,
      SourceEditor) then
      Exit;

    Filename := SourceEditor.FileName;
    Extension := LowerCase(ExtractFileExt(Filename));
    if (Extension <> '.pas') and (Extension <> '.dpk') and (Extension <> '.dpr')
      and (Extension <> '.dpkw') then
      Exit;
    ...
    try
      ...
      try
        ...
        begin
          if FFormatter.Settings.FillNewWords in [fmAddNewWord, fmAddUse,
            fmAddUseExcept] then
...
procedure TKeyboardNotifier.BindKeyboard(const BindingServices:
